...
export const makeFetchLikeRequest = (fetch: Fetch, url: string, upload: string) => fetch(`${url}/git-upload-pack`, {
    method: 'POST',
    body: upload,
    headers: {
...
export const httpFetchUsing = (fetch: Fetch) => (url: string) => (upload: string) => makeFetchLikeRequest(fetch, url, upload);
...
export interface ShallowCloneCommitOptions {
    makeRequest: (request: string) => Promise<Buffer>;
...
export async function shallowCloneCommit(commit: string, { makeRequest, filter }: ShallowCloneCommitOptions) {
    ...
    const requestAndReadPackFile = (request: string) =>
        makeRequest(request).then(readResponse).then(readPack);
